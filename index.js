...
const { default: connectionOptions, useMultiFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("darkleys")
...
if (!dark.public && !itsMedark && chatUpdate.type === 'notify') return
...
//Jangan Di Hapus
dark.sendTextWithMentions = async (jid, text, quoted, options = {}) => dark.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
...
dark.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
               ...
               if (anu.action == 'add') {
                    dark.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Welcome @${num.split("@")[0]} To Group ${metadata.subject} 👋` })
                } else if (anu.action == 'remove') {
                    dark.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Sayonaraa @${num.split("@")[0]} 👋` })
                } else if (anu.action == 'promote') {
                    dark.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Selamat Ya @${num.split("@")[0]} Atas Kenaikan Jabatannya Di Grup ${metadata.subject} 🎉` })
                } else if (anu.action == 'demote') {
                    dark.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Nice Try @${num.split("@")[0]} Atas Penurunan Jabatannya Di Grup ${metadata.subject} 😔` })
              }
            }
        } catch (err) {
...
dark.serializeM = (m) => smsg(dark, m, store)
    ...
    dark.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, reconnecting..."); darkBot(); }
        ...
        if (update.connection == "open" || update.receivedPendingNotifications == "true") {
         ...
         console.log(`Connected to = ` + JSON.stringify(dark.user, null, 2))
         //dark.sendMessage("77777777777" + "@s.whatsapp.net", {text:"", "contextInfo":{"expiration": 86400}})
      }
    })

    dark.ev.on('creds.update', saveCreds)
dark.sendText = (jid, text, quoted = '', options) => dark.sendMessage(jid, {
      text: text,
      ...options
   }, {
      quoted
